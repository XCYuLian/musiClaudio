...
async function buildContext(opts = {}) {
  ...
  const systemPrompt = [
    ...
    '- `play` MUST contain exactly 1 search query in format "Artist - SongName". Single-track DJ: recommend ONE song at a time.',
    '- `monologue` MUST be brief (≤50 Chinese chars). DJ-style segue like "刚刚那首如何？接下来..."',
    '- `reason` MUST explain why THIS track fits THIS exact moment.',
    '- `segue` should bridge the previous track into this one.',
    '',
    'CRITICAL: Use the CURRENT TIME block above as the authoritative time reference. Do NOT guess or invent time values.',
    '',
    '## DISCOVERY RULE (70/30)',
    'Your primary mission is MUSIC DISCOVERY. Before recommending ANY track, check against the playlist in <MANDATORY_USER_DNA>.',
    '70% of the time: recommend NEW songs NOT in their playlist — hidden gems matching their DNA.',
    '30% of the time: familiar tracks from their playlist — comfort picks.',
    'Each recommendation is for ONE track. Alternate between discovery and comfort.',
