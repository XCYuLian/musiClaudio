  ...
  try {
    ...
    if (result.play?.length) {
      try {
        ...
        console.log(`[scheduler] Resolved ${tracks.length}/${result.play.length} tracks`);
      } catch (err) {
        console.error('[scheduler] Track resolution failed:', err.message);
      }
    }

    // Persist
    state.addMessage('system', `[${trigger}]`);
    const displayText = result.monologue || result.reply || result.say || '';
    state.addMessage('assistant', displayText, {
      play: result.play,
      reason: result.reason,
      segue: result.segue,
    });

    // Synthesize speech (monologue for broadcast, or reply for direct answer)
    const speechText = result.monologue || result.reply || result.say || '';
    const ttsUrl = await synthesize(speechText).catch(err => {
    ...
    const logText = result.monologue || result.reply || result.say || '';
    console.log(`[scheduler] Task complete: ${trigger} → "${logText.slice(0, 50)}…"`);
    return result;
  } catch (err) {
