...
// DB location: Electron userData > pkg exe dir > project root
let DB_PATH;
if (process.versions && process.versions.electron) {
  const { app } = require('electron');
  DB_PATH = path.join(app.getPath('userData'), 'state.db');
} else if (process.pkg) {
  DB_PATH = path.join(path.dirname(process.execPath), 'state.db');
} else {
  DB_PATH = path.resolve(__dirname, '..', 'state.db');
}
