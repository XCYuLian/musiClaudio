...
const PROXY_PORT = parseInt(process.env.UNBLOCK_PORT) || 8081;
const PROXY_HOST = '127.0.0.1';
const PROXY_URL = `http://${PROXY_HOST}:${PROXY_PORT}`;

let child = null;
let online = false;
let onStatus = null; // callback(status)
...
function start() {
  ...
  const args = [
    ...
    '-o', 'kugou qq migu kuwo',  // source priority
  ];
  ...
  child.stdout.on('data', (d) => {
    ...
    if (!online && msg.includes('listening') || msg.includes('server')) {
      online = true;
