...
async function start() {
  ...
  // Force-free port
  try {
    execSync(`netstat -ano | findstr :${PROXY_PORT||8081} | findstr LISTENING`, { encoding: 'utf8', timeout: 3000 });
    const out = execSync(`netstat -ano | findstr :${PROXY_PORT||8081} | findstr LISTENING`, { encoding: 'utf8' });
    const pid = out.trim().split(/\s+/).pop();
    if (pid) { execSync(`taskkill /F /PID ${pid}`); console.log(`[proxy] Killed PID ${pid} holding port ${PROXY_PORT||8081}`); }
  } catch {}
  ...
  const appPath = path.join(__dirname, '..', 'node_modules', '@unblockneteasemusic', 'server', 'src', 'app.js');
  const args = [
    appPath,
    '-p', String(PROXY_PORT),
  ...
  // Minimal env — only what Node needs
  const cleanEnv = { SYSTEMROOT: process.env.SYSTEMROOT, PATH: process.env.PATH, TEMP: process.env.TEMP };
