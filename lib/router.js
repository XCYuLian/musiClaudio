...
// ── Intent classification: question vs chat vs music ──
const QUESTION_RE = /[？?]|什么|怎么|为什么|谁|哪|吗|呢|多少|能不能|可以|是否|有没有|看过|听过|知道|觉得|查|帮我|告诉我|解释/i;

function classifyIntent(message) {
  const m = message.trim();
  // Explicit music request
  if (MUSIC_QUERY_RE.test(m)) return 'music';
  // Contains question patterns
  if (QUESTION_RE.test(m)) return 'question';
  // Short casual chat
  if (m.length < 20 && !/[，。！]/.test(m)) return 'question';
  return 'chat';
}
...
async function route(message) {
  ...
  for (const cmd of INTENTS) {
    ...
    if (match) {
      return handleCommand(cmd.name, match[1] || '');
    }
  }
  ...
  // ── Step 3: classify intent → LLM pipeline ──
  const intent = classifyIntent(trimmed);
  return handleLLM(trimmed, intent);
}
...
async function handleMusicSearch(query) {
  ...
  if (ncmError || tracks.length === 0) {
    ...
    return handleLLM(`我想听 ${query}`, 'music');
  }

  const resultsText = tracks.slice(0, 5).map((t, i) => `${i + 1}. ${t.label}`).join('\n');
  ...
  state.addMessage('assistant', result.monologue || result.reply || result.say || '', { play: result.play, reason: result.reason });

  return {
    type: 'search',
    reply: result.reply || '',
    monologue: result.monologue || result.say || '',
    play: result.play,
...
async function handleLLM(userInput, intent = 'chat') {
  const currentState = state.getState();
  ...
  state.addMessage('assistant', result.monologue || result.reply || result.say || '', {
    play: result.play,
    reason: result.reason,
    segue: result.segue,
  });
