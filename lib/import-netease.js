...
async function importPlaylists(uid, cookie = '', options = {}, onProgress = null) {
  ...
  // 3. Fetch tracks with pagination (500 per batch, up to maxTracks)
  const allTracks = [];
  for (let i = 0; i < targets.length; i++) {
    const pl = targets[i];
    const name = pl.name || `playlist_${pl.id}`;
    const total = pl.trackCount || 0;
    log('tracks', `Fetching: ${name} (${total} total)`, i + 1, targets.length);

    try {
      let offset = 0;
      const PAGE = 500;
      while (allTracks.length < maxTracks && offset < total) {
        const detail = await getPlaylistDetail(pl.id, PAGE, offset);
        const tracks = detail?.tracks || [];
        if (!tracks.length) break;
        allTracks.push(...tracks);
        offset += PAGE;
        log('tracks', `${name}: ${allTracks.length} / ${Math.min(total, maxTracks)}`, i + 1, targets.length);
        if (tracks.length < PAGE) break; // last page
      }
    } catch (err) {
